...
COMPACT_CELL_VOID = 0
COMPACT_CELL_WATER = 3
WOOD_WALL_TYPE_ID = 4
STONE_WALL_TYPE_ID = 5
...
def build_systems(data: dict[str, Any], turn_records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "terrain": build_terrain_system(data, turn_records),
...
def build_terrain_system(
    ...
    piece_catalog = index_by_int_id(as_list(path_get(data, "referenceData", "pieceTypes")))
    max_range = to_int(path_get(data, "configContext", "combat", "globalMaxRange"), 8) or 8
    first_snapshot = as_dict(turn_records[0].get("snapshot"))
    first_grid = as_list(first_snapshot.get("grid"))
    total_in_circle_count, traversable_non_water_count, water_cell_count = count_compact_grid_cells(
        first_grid
    )

    per_record_rows = []
    by_kingdom_rows: dict[str, list[dict[str, Any]]] = defaultdict(list)
    dry_totals_by_piece_type: Counter[int] = Counter()
    lost_totals_by_piece_type: Counter[int] = Counter()
    piece_samples_by_type: Counter[int] = Counter()
    impacted_piece_samples_by_type: Counter[int] = Counter()
    wall_breach_piece_samples = 0

    for record in turn_records:
        snapshot = as_dict(record.get("snapshot"))
        grid = as_list(snapshot.get("grid"))
        pieces_by_position, buildings_by_cell, autonomous_by_position = build_compact_snapshot_state(
            snapshot
        )
        active_pieces = get_compact_snapshot_kingdom_pieces(
            snapshot, record["committedActiveKingdomKey"]
        )

        dry_destination_count = 0
        actual_destination_count = 0
        lost_destination_count = 0
        impacted_piece_count = 0

        for piece in active_pieces:
            if not isinstance(piece, dict):
                continue
            if piece.get("hasWallBreachEntry"):
                wall_breach_piece_samples += 1

            wet_moves = set(
                compute_compact_piece_moves(
                    piece,
                    grid,
                    pieces_by_position,
                    buildings_by_cell,
                    autonomous_by_position,
                    max_range,
                    treat_water_as_ground=False,
                )
            )
            dry_moves = set(
                compute_compact_piece_moves(
                    ...
                    autonomous_by_position,
                    max_range,
                    treat_water_as_ground=True,
                )
            )
            lost_here = len(dry_moves - wet_moves)
            dry_here = len(dry_moves)
            piece_type_id = to_int(piece.get("type"), None)
            if piece_type_id is not None:
                piece_samples_by_type[piece_type_id] += 1
                dry_totals_by_piece_type[piece_type_id] += dry_here
                lost_totals_by_piece_type[piece_type_id] += lost_here
                if lost_here > 0:
                    impacted_piece_samples_by_type[piece_type_id] += 1

            dry_destination_count += dry_here
            actual_destination_count += len(wet_moves)
            lost_destination_count += lost_here
            if lost_here > 0:
                impacted_piece_count += 1

        row = {
            "sequenceIndex": record["recordSequenceIndex"],
            "turn": record["turn"],
            "committedActiveKingdomKey": record["committedActiveKingdomKey"],
            "activePieceCount": len(active_pieces),
            "impactedPieceCount": impacted_piece_count,
            "impactedPieceRatio": safe_div(impacted_piece_count, len(active_pieces)),
            "actualReachableDestinationCount": actual_destination_count,
            "dryReachableDestinationCount": dry_destination_count,
            "lostDestinationCount": lost_destination_count,
            "lostMobilityShare": safe_div(lost_destination_count, dry_destination_count),
            "averageLostDestinationsPerActivePiece": safe_div(
                lost_destination_count, len(active_pieces)
            ),
        }
        per_record_rows.append(row)
        by_kingdom_rows[record["committedActiveKingdomKey"]].append(row)

    ratio_rows = [
        row["lostMobilityShare"]
        for row in per_record_rows
        if isinstance(row.get("lostMobilityShare"), (int, float))
    ]
    impacted_ratio_rows = [
        row["impactedPieceRatio"]
        for row in per_record_rows
        if isinstance(row.get("impactedPieceRatio"), (int, float))
    ]

    kingdom_summary_rows = []
    for kingdom_key, rows in sorted(by_kingdom_rows.items()):
        kingdom_summary_rows.append(
            {
                "kingdomKey": kingdom_key,
                "recordCount": len(rows),
                "averageLostDestinationCount": safe_mean(
                    [row["lostDestinationCount"] for row in rows]
                ),
                "averageLostMobilityShare": safe_mean(
                    [row["lostMobilityShare"] for row in rows if row["lostMobilityShare"] is not None]
                ),
                "averageImpactedPieceCount": safe_mean(
                    [row["impactedPieceCount"] for row in rows]
                ),
                "averageImpactedPieceRatio": safe_mean(
                    [row["impactedPieceRatio"] for row in rows if row["impactedPieceRatio"] is not None]
                ),
            }
        )

    piece_type_rows = []
    for piece_type_id in sorted(piece_samples_by_type):
        piece_info = as_dict(piece_catalog.get(piece_type_id))
        piece_type_rows.append(
            {
                "pieceTypeId": piece_type_id,
                "pieceTypeKey": key_or_fallback(piece_info.get("key"), str(piece_type_id)),
                "pieceTypeLabel": key_or_fallback(
                    piece_info.get("label"), key_or_fallback(piece_info.get("key"), str(piece_type_id))
                ),
                "activePieceSampleCount": piece_samples_by_type[piece_type_id],
                "impactedPieceSampleCount": impacted_piece_samples_by_type[piece_type_id],
                "dryReachableDestinationCount": dry_totals_by_piece_type[piece_type_id],
                "lostDestinationCount": lost_totals_by_piece_type[piece_type_id],
                "lostMobilityShare": safe_div(
                    lost_totals_by_piece_type[piece_type_id],
                    dry_totals_by_piece_type[piece_type_id],
                ),
                "averageLostDestinationsPerActivePieceSample": safe_div(
                    lost_totals_by_piece_type[piece_type_id],
                    piece_samples_by_type[piece_type_id],
                ),
                "impactedPieceSampleRatio": safe_div(
                    impacted_piece_samples_by_type[piece_type_id],
                    piece_samples_by_type[piece_type_id],
                ),
            }
        )

    top_pressure_records = sorted(
        per_record_rows,
        key=lambda row: (-row["lostDestinationCount"], row["sequenceIndex"]),
    )[:10]

    return {
        "summary": {
            "globalMaxRange": max_range,
            "totalInCircleCellCount": total_in_circle_count,
            "traversableNonWaterCellCount": traversable_non_water_count,
            "waterCellCount": water_cell_count,
            "waterCoverageOfInCircle": safe_div(water_cell_count, total_in_circle_count),
            "averageActiveLostDestinationCount": safe_mean(
                [row["lostDestinationCount"] for row in per_record_rows]
            ),
            "averageActiveLostMobilityShare": safe_mean(ratio_rows),
            "averageImpactedPieceCount": safe_mean(
                [row["impactedPieceCount"] for row in per_record_rows]
            ),
            "averageImpactedPieceRatio": safe_mean(impacted_ratio_rows),
            "maxLostDestinationCount": max_or_default(
                [row["lostDestinationCount"] for row in per_record_rows], 0
            ),
            "wallBreachPieceSampleCount": wall_breach_piece_samples,
            "method": "Compare les destinations atteignables sur le snapshot courant avec et sans blocage eau, a occupation identique.",
        },
        "perRecordActiveKingdom": per_record_rows,
        "byKingdom": kingdom_summary_rows,
        "byPieceType": piece_type_rows,
        "phaseRows": build_terrain_phase_rows(per_record_rows),
        "topPressureRecords": top_pressure_records,
    }
...
def count_compact_grid_cells(grid: list[Any]) -> tuple[int, int, int]:
    total_in_circle_count = 0
    traversable_non_water_count = 0
    water_cell_count = 0
    for row in grid:
        for cell in as_list(row):
            if not isinstance(cell, dict) or not bool(cell.get("c")):
                continue
            total_in_circle_count += 1
            terrain_type = to_int(cell.get("t"), COMPACT_CELL_VOID)
            if terrain_type == COMPACT_CELL_WATER:
                water_cell_count += 1
            elif terrain_type != COMPACT_CELL_VOID:
                traversable_non_water_count += 1
    return total_in_circle_count, traversable_non_water_count, water_cell_count


def build_compact_snapshot_state(
    snapshot: dict[str, Any],
) -> tuple[dict[tuple[int, int], dict[str, Any]], dict[tuple[int, int], dict[str, Any]], dict[tuple[int, int], dict[str, Any]]]:
    pieces_by_position: dict[tuple[int, int], dict[str, Any]] = {}
    buildings_by_cell: dict[tuple[int, int], dict[str, Any]] = {}
    autonomous_by_position: dict[tuple[int, int], dict[str, Any]] = {}

    for kingdom_container_key in ("whiteKingdom", "blackKingdom"):
        kingdom = as_dict(snapshot.get(kingdom_container_key))
        for piece in as_list(kingdom.get("pieces")):
            if not isinstance(piece, dict):
                continue
            x = to_int(piece.get("x"), None)
            y = to_int(piece.get("y"), None)
            if x is not None and y is not None:
                pieces_by_position[(x, y)] = piece
        for building in as_list(kingdom.get("buildings")):
            register_compact_building_cells(buildings_by_cell, building)

    for building in as_list(snapshot.get("publicBuildings")):
        register_compact_building_cells(buildings_by_cell, building)
    for autonomous_unit in as_list(snapshot.get("autonomousUnits")):
        if not isinstance(autonomous_unit, dict):
            continue
        x = to_int(autonomous_unit.get("x"), None)
        y = to_int(autonomous_unit.get("y"), None)
        if x is not None and y is not None:
            autonomous_by_position[(x, y)] = autonomous_unit

    return pieces_by_position, buildings_by_cell, autonomous_by_position


def register_compact_building_cells(
    target: dict[tuple[int, int], dict[str, Any]], building: Any
) -> None:
    if not isinstance(building, dict):
        return
    origin_x = to_int(building.get("ox"), None)
    origin_y = to_int(building.get("oy"), None)
    width = to_int(building.get("w"), None)
    height = to_int(building.get("h"), None)
    if origin_x is None or origin_y is None or width is None or height is None:
        return

    hp = as_list(building.get("hp"))
    for local_y in range(height):
        for local_x in range(width):
            index = (local_y * width) + local_x
            target[(origin_x + local_x, origin_y + local_y)] = {
                "building": building,
                "destroyed": index < len(hp) and to_int(hp[index], 0) <= 0,
            }


def get_compact_snapshot_kingdom_pieces(
    snapshot: dict[str, Any], kingdom_key: str
) -> list[Any]:
    container_key = {
        "white": "whiteKingdom",
        "black": "blackKingdom",
    ...
    if container_key is None:
        return []
    return as_list(as_dict(snapshot.get(container_key)).get("pieces"))


def build_terrain_phase_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not rows:
        return []

    ordered_rows = sorted(rows, key=lambda row: row["sequenceIndex"])
    phase_rows = []
    total = len(ordered_rows)
    for phase_index in range(4):
        start = int(phase_index * total / 4)
        end = int((phase_index + 1) * total / 4)
        chunk = ordered_rows[start:end]
        phase_rows.append(
            {
                "quartile": f"Q{phase_index + 1}",
                "recordCount": len(chunk),
                "averageLostDestinationCount": safe_mean(
                    [row["lostDestinationCount"] for row in chunk]
                ),
                "averageLostMobilityShare": safe_mean(
                    [row["lostMobilityShare"] for row in chunk if row["lostMobilityShare"] is not None]
                ),
                "averageImpactedPieceCount": safe_mean(
                    [row["impactedPieceCount"] for row in chunk]
                ),
                "averageImpactedPieceRatio": safe_mean(
                    [row["impactedPieceRatio"] for row in chunk if row["impactedPieceRatio"] is not None]
                ),
            }
        )
    return phase_rows
...
def compute_compact_piece_moves(
    piece: dict[str, Any],
    grid: list[Any],
    pieces_by_position: dict[tuple[int, int], dict[str, Any]],
    buildings_by_cell: dict[tuple[int, int], dict[str, Any]],
    autonomous_by_position: dict[tuple[int, int], dict[str, Any]],
    max_range: int,
    treat_water_as_ground: bool,
) -> list[tuple[int, int]]:
    piece_type = to_int(piece.get("type"), None)
    mover = to_int(piece.get("kingdom"), None)
    origin_x = to_int(piece.get("x"), None)
    origin_y = to_int(piece.get("y"), None)
    if piece_type is None or mover is None or origin_x is None or origin_y is None:
        return []

    if piece_type == 0:
        return compute_compact_pawn_moves(
            piece,
            grid,
            pieces_by_position,
            buildings_by_cell,
            autonomous_by_position,
            treat_water_as_ground,
        )
    if piece_type == 1:
        return compute_compact_knight_moves(
            piece,
            grid,
            pieces_by_position,
            buildings_by_cell,
    ...
    if piece_type == 2:
        ...
        return compute_compact_king_moves(
            piece,
            grid,
            pieces_by_position,
            buildings_by_cell,
...
def compute_compact_pawn_moves(
    piece: dict[str, Any],
    grid: list[Any],
    pieces_by_position: dict[tuple[int, int], dict[str, Any]],
    buildings_by_cell: dict[tuple[int, int], dict[str, Any]],
    ...
    origin_x = to_int(piece.get("x"), 0)
    origin_y = to_int(piece.get("y"), 0)
    mover = to_int(piece.get("kingdom"), -1)
    moves: list[tuple[int, int]] = []

    for delta_x, delta_y in ((0, 1), (0, -1), (1, 0), (-1, 0)):
        target_x = origin_x + delta_x
        target_y = origin_y + delta_y
        if not is_compact_destination_traversable(
            grid, target_x, target_y, treat_water_as_ground
        ):
            continue
        piece_at = pieces_by_position.get((target_x, target_y))
        autonomous_at = autonomous_by_position.get((target_x, target_y))
        building_entry = buildings_by_cell.get((target_x, target_y))
        if is_allied_blocking_wall_entry(building_entry, mover):
            if piece_at is None and autonomous_at is None:
                moves.append((target_x, target_y))
            continue
        if piece_at is not None or autonomous_at is not None:
            continue
        if is_enemy_capturable_building_entry(building_entry, mover):
            continue
        if is_blocking_wall_entry(building_entry):
            continue
        moves.append((target_x, target_y))
    ...
    for delta_x, delta_y in ((1, 1), (1, -1), (-1, 1), (-1, -1)):
        ...
        autonomous_at = autonomous_by_position.get((target_x, target_y))
        building_entry = buildings_by_cell.get((target_x, target_y))
        if piece_at is not None and to_int(piece_at.get("kingdom"), -1) != mover:
            moves.append((target_x, target_y))
            continue
        if autonomous_at is not None:
            moves.append((target_x, target_y))
            continue
        if is_enemy_capturable_building_entry(building_entry, mover):
            moves.append((target_x, target_y))
...
def compute_compact_knight_moves(
    piece: dict[str, Any],
    grid: list[Any],
    pieces_by_position: dict[tuple[int, int], dict[str, Any]],
    buildings_by_cell: dict[tuple[int, int], dict[str, Any]],
    ...
    for delta_x, delta_y in (
        ...
        if is_blocking_wall_entry(building_entry):
            if not (
                piece_at is not None and to_int(piece_at.get("kingdom"), -1) == mover
            ) and not bool(as_dict(building_entry.get("building")).get("isNeutral")):
                moves.append((target_x, target_y))
            continue
        if piece_at is not None and to_int(piece_at.get("kingdom"), -1) == mover:
            continue
...
def compute_compact_king_moves(
    piece: dict[str, Any],
    grid: list[Any],
    pieces_by_position: dict[tuple[int, int], dict[str, Any]],
    buildings_by_cell: dict[tuple[int, int], dict[str, Any]],
    ...
    for delta_y in (-1, 0, 1):
        for delta_x in (-1, 0, 1):
            ...
            if is_allied_blocking_wall_entry(building_entry, mover):
                ...
                continue
            if is_blocking_wall_entry(building_entry):
                building = as_dict(building_entry.get("building"))
                if to_int(building.get("owner"), -1) != mover and not bool(
                    building.get("isNeutral")
                ):
                    moves.append((target_x, target_y))
                continue
            if piece_at is not None and to_int(piece_at.get("kingdom"), -1) == mover:
...
def trace_compact_direction(
    ...
    for step in range(1, max_range + 1):
        target_x = origin_x + (delta_x * step)
        target_y = origin_y + (delta_y * step)
        if not is_compact_destination_traversable(
            grid, target_x, target_y, treat_water_as_ground
        ):
            break
...
def is_compact_destination_traversable(
    grid: list[Any], x: int, y: int, treat_water_as_ground: bool
) -> bool:
    ...
    if x < 0 or x >= len(row):
        return False
    cell = as_dict(row[x])
    if not bool(cell.get("c")):
        return False
    terrain_type = to_int(cell.get("t"), COMPACT_CELL_VOID)
    if terrain_type == COMPACT_CELL_VOID:
        return False
    if terrain_type == COMPACT_CELL_WATER and not treat_water_as_ground:
        return False
    return True


def is_blocking_wall_entry(entry: dict[str, Any] | None) -> bool:
    if not isinstance(entry, dict):
        return False
    building = as_dict(entry.get("building"))
    building_type = to_int(building.get("type"), -1)
    return building_type in {WOOD_WALL_TYPE_ID, STONE_WALL_TYPE_ID} and not bool(
        entry.get("destroyed")
    )


def is_allied_blocking_wall_entry(entry: dict[str, Any] | None, mover: int) -> bool:
    if not is_blocking_wall_entry(entry):
        return False
    building = as_dict(entry.get("building"))
    return not bool(building.get("isNeutral")) and to_int(building.get("owner"), -1) == mover


def is_enemy_capturable_building_entry(entry: dict[str, Any] | None, mover: int) -> bool:
    if not isinstance(entry, dict):
        return False
    building = as_dict(entry.get("building"))
    return not bool(building.get("isNeutral")) and to_int(building.get("owner"), -1) != mover
