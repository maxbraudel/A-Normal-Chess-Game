...
void testSessionConfigDefaults() {
    GameSessionConfig session = makeDefaultGameSessionConfig(GameMode::HumanVsHuman, "session_test");

    expect(controllerFor(session, KingdomId::White) == ControllerType::Human,
        "White kingdom should default to a human in the human-only preset.");
    expect(controllerFor(session, KingdomId::Black) == ControllerType::Human,
        "Black kingdom should default to a human in the human-only preset.");
    expect(gameModeFromSession(session) == GameMode::HumanVsHuman,
...
void testGameEngineRestoresBishopSpawnMemory() {
    ...
    void testGameEngineAssignsWorldSeed() {
        GameConfig config;
        GameEngine engine;
        GameSessionConfig session = makeDefaultGameSessionConfig(GameMode::HumanVsHuman, "engine_world_seed_test");
        session.worldSeed = 0;
